...
// For debugging, it's helpful to see errors. You can comment these out in production.
...
if (!file_exists($vendor_autoload)) {
    // It's good practice to log errors for your own review.
...
$case_id      = mysqli_real_escape_string($connect, $case_id);
$place        = mysqli_real_escape_string($connect, $place);
$floor        = mysqli_real_escape_string($connect, $floor);
$room         = mysqli_real_escape_string($connect, $room);
$explane      = mysqli_real_escape_string($connect, $explane);
$problem_type = mysqli_real_escape_string($connect, $problem_type);
$description  = mysqli_real_escape_string($connect, $description);
$escaped_img  = mysqli_real_escape_string($connect, $escaped_img);

$query = "INSERT INTO report (case_id, place, floor, room, explane, problem_type, description, img)
          VALUES ('$case_id', '$place', '$floor', '$room', '$explane', '$problem_type', '$description', '$escaped_img')";

if (mysqli_query($connect, $query)) {
    $recipient_email = '44358@bodin.ac.th';
    if (filter_var($recipient_email, FILTER_VALIDATE_EMAIL)) {
        $subject = 'ยืนยันการรับเรื่อง: ' . $case_id;
        $headers = "MIME-Version: 1.0" . "\r\n";
        $headers .= "Content-type:text/html;charset=UTF-8" . "\r\n";
        $headers .= "From: ระบบแจ้งซ่อม ร.ร.บดินทรเดชา <nachakhun1998@gmail.com>" . "\r\n";
        ...
        <style>
            body {font-family: "Sarabun", "Segoe UI", Arial, sans-serif; line-height: 1.6; color: #333; background-color: #f5f5f5; margin: 0; padding: 0;}
            ...
            .header .school-name {font-size: 14px; opacity: 0.9; margin-top: 5px;}
            ...
            .case-id {background: #e3f2fd; border-left: 4px solid #2196f3; padding: 15px; margin: 20px 0; border-radius: 0 8px 8px 0;}
            .case-id strong {color: #1976d2; font-size: 18px;}
            ...
            .details-table td {padding: 12px 15px; vertical-align: top;}
            .details-table .label {background: #f8f9fa; font-weight: 600; color: #495057; width: 150px; border-right: 2px solid #dee2e6;}
            .details-table .value {color: #212529;}
            .status-success {color: #28a745; font-weight: 600; padding: 4px 8px; background: #d4edda; border-radius: 4px;}
            .next-steps {background: #fff3cd; border: 1px solid #ffeaa7; border-radius: 8px; padding: 20px; margin: 25px 0;}
            .next-steps h3 {margin-top: 0; color: #856404; font-size: 16px;}
            .next-steps ul {margin: 10px 0; padding-left: 20px;}
            .next-steps li {margin-bottom: 8px; color: #856404;}
            .footer {background: #454545ff; padding: 20px; text-align: center; border-top: 1px solid #dee2e6; color: #ffffffff; font-size: 14px;}
            .footer .contact-info {margin-top: 15px; font-size: 13px;}
            .divider {height: 2px; background: linear-gradient(90deg, #667eea, #764ba2); margin: 20px 0; border-radius: 2px;}
        </style>
        </head>
        <body>
            <div class="container">
                ...
                <div class="content">
                    ...
                    <p style="text-align: center; font-size: 16px; color: #28a745; font-weight: 600;">ขอขอบคุณที่ใช้บริการระบบแจ้งซ่อม</p>
                </div>
                <div class="footer">
        ...
        $email_sent = mail($recipient_email, $subject, $body, $headers);
    }
    ...
    echo json_encode([
        'success' => false,
        'message' => 'เกิดข้อผิดพลาดในการบันทึกข้อมูล: ' . mysqli_error($connect)
    ]);
}
