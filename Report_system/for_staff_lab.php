...
<body>
    ...
    <table class = "Report-box">
        ...
        <tbody>
            <?php 
                $query = "SELECT case_id, problem_type, place, report_date, status FROM report ORDER BY report_date DESC";
                ...
                if (mysqli_num_rows($result) > 0) {
                    while ($row = mysqli_fetch_assoc($result)) {
                        ...
                        echo "<td class = 'status'>" . htmlspecialchars($row['status']) . '
                            <div class="case-info">
                                <button popovertarget = "' . $infoId . '" class = icon-button> <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6" style="width: 1.5rem; height: 1.5rem; vertical-align: middle; margin-left: 5px;">
                                ...
                                <div id="' . $infoId . '" class="case-detail" popover>                                                    
                                    ...
                                    <div class = "line"></div>
                                    <p class = "case-id">  รหัสเคส :  '. htmlspecialchars($row['case_id']) . '</p>
